...
function CategoryProductItem(props) {
    ...
    return (
        <div className="col l-3 product-list-item">
                <Link to ="/" className="link product-item_img">
                    <img src={products.img} alt="product_img"/>
                </Link>
                <div className="product-item-body">
                <Link to ={"/product-detail/" +chuyenDoiURL(products.name) + "." + products.id +".html"} className="link product-item-name">{products.name}</Link>
                    <div className="product-item-price">
                        <span>{(products.price).toLocaleString('vi-VN')} <sup>đ</sup></span>
                    </div>
